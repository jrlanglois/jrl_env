...
def getVariantPatterns(variant):
...
def trySingleUrl(url, filePath, timeout=5):
...
def tryGithubRepo(fontName, variant, tempDir):
...
def tryGoogleFontsApi(fontName, variant, tempDir):
...
def convertWoff2ToTtf(woff2Path, ttfPath):
...
def downloadFont(fontName, variant, tempDir):
...
def installFontFile(fontPath, installDir):
...
def downloadFontVariant(fontName, variant, tempDir):
...
def convertFontFile(fontName, variant, filePath, tempDir):
...
def verifyFontInstalled(fontPath, installDir):
...
def installFontVariant(fontName, variant, filePath, installDir, results):
...
def refreshFontCache(installDir):
...
def main():
    """Main function"""
    if len(sys.argv) < 3:
        print(f"Usage: {sys.argv[0]} <configPath> <installDir> [variants...] [--dry-run]")
        print(f"  --dry-run: Show what would be downloaded/installed without actually doing it")
        sys.exit(1)
    ...
    if dryRun:
        printInfo("=== Google Fonts Installation (DRY RUN) ===")
    else:
        printInfo("=== Google Fonts Installation ===")
        ...
        for future in as_completed(downloadFutures):
            ...
            try:
                ...
                if result:
                    ...
                    printSuccess(f"  [{downloadCount}/{totalDownloads}] ✓ {fontName} {variant}")
                else:
                    # Don't show failures for every variant to reduce noise
                    pass
            except Exception as e:
                downloadCount += 1
                printError(f"  [{downloadCount}/{totalDownloads}] ✗ {fontName} {variant}: {e}")
    ...
    # Show all "Installing" messages first
    for fontName in sorted(fontsByName.keys()):
        printWarning(f"Installing: {fontName}")
    
    safePrint("")
    
    # Collect all installation results
    installResults = []
    ...
    with ThreadPoolExecutor(max_workers=maxWorkers) as executor:
        ...
        for fontName, variantsList in fontsByName.items():
            for variant, filePath in variantsList:
                future = executor.submit(installFontVariant, fontName, variant, filePath, installDir, results)
                installFutures.append(future)
        ...
        for future in as_completed(installFutures):
            try:
                ...
                installResults.append((False, "Unknown", "Unknown", str(e)))
