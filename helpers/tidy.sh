...
tidyFile()
{
    ...
    if grep -q '[[:space:]]$' "$filePath" 2>/dev/null; then
...
if [ "${BASH_SOURCE[0]}" = "${0}" ]; then
    ...
    if [ "$2" = "--dry-run" ] || [ "$2" = "--dryRun" ] || [ "$2" = "-d" ]; then
        isDryRun=true
    fi
