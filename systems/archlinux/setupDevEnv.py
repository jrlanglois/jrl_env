#!/usr/bin/env python3
"""
Script to set up ArchLinux development environment.
...
osConfigPath = str(projectRoot / "configs" / "archlinux.json")
...
def setupDevEnv(dryRun: bool = False) -> bool:
    """Main setup function."""
    printSection("ArchLinux Development Environment Setup", dryRun=dryRun)
    safePrint()
    ...
    if not installZsh(
        dryRun=dryRun,
        installFunc=installZshPacman,
        dryRunMessage="[DRY RUN] Would install zsh via pacman...",
    ):
        success = False
    safePrint()
