#!/usr/bin/env python3
"""
Script to set up OpenSUSE development environment.
...
osConfigPath = str(projectRoot / "configs" / "opensuse.json")
...
def installZshZypper() -> bool:
    """Install zsh via zypper."""
    try:
        subprocess.run(
            ["sudo", "zypper", "refresh"],
            check=False,
...
def setupDevEnv(dryRun: bool = False) -> bool:
    """Main setup function."""
    printSection("OpenSUSE Development Environment Setup", dryRun=dryRun)
    safePrint()
    ...
    if not installZsh(
        dryRun=dryRun,
        installFunc=installZshZypper,
        dryRunMessage="[DRY RUN] Would install zsh via zypper...",
    ):
        success = False
    safePrint()
