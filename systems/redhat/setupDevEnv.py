#!/usr/bin/env python3
"""
Script to set up RedHat development environment.
...
osConfigPath = str(projectRoot / "configs" / "redhat.json")
...
def setupDevEnv(dryRun: bool = False) -> bool:
    """Main setup function."""
    printSection("RedHat Development Environment Setup", dryRun=dryRun)
    safePrint()
    ...
    if not installZsh(
        dryRun=dryRun,
        installFunc=installZshDnf,
        dryRunMessage="[DRY RUN] Would install zsh via dnf...",
    ):
        success = False
    safePrint()
