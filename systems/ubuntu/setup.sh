...
# Initialize logging
logFile=$(initLogging)
logInfo "=== jrl_env Setup for Ubuntu ==="
...
backupConfigs()
{
    ...
    backupDir="/tmp/jrl_env_backup_$(date +%Y%m%d_%H%M%S)"
    mkdir -p "$backupDir"
    ...
    cursorSettings="$HOME/.config/Cursor/User/settings.json"
...
checkDependencies()
{
    ...
    if ! command -v jq >/dev/null 2>&1; then
        missing+=("jq")
        logWarn "jq is not installed"
    else
        logSuccess "jq is installed"
    fi
...
# 1. Setup development environment (zsh, Oh My Zsh)
...
if [ "$runApps" = true ]; then
    if [ "$dryRun" = true ]; then
        ...
        logInfo "Would install/update apps from ubuntu.json"
    else
            ...
            if [ -n "$workPath" ] && [ "$workPath" != "null" ]; then
                ...
                if [ -d "$workPath" ]; then
                    ...
                    if [ "$ownerDirs" -gt 0 ]; then
                        ...
                        logInfo "To clone repositories manually, run: ./systems/ubuntu/cloneRepositories.sh"
                    else
