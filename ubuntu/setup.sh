...
echo -e "${cyan}=== jrl_env Setup for Ubuntu ===${nc}"
echo ""
...
if [ "$runApps" = true ]; then
    if [ "$dryRun" = true ]; then
        ...
        echo -e "Would install/update apps from ubuntuApps.json"
    else
            ...
            if [ -n "$workPath" ] && [ "$workPath" != "null" ]; then
                ...
                if [ -d "$workPath" ]; then
                    ...
                    if [ "$ownerDirs" -gt 0 ]; then
                        ...
                        echo -e "To clone repositories manually, run: ./ubuntu/cloneRepositories.sh"
                        echo ""
                    else
